...
在 macOS 终端中下载 YouTube 视频、压缩本地视频，或烧录本地 SRT 字幕。按 **Tab** 切换模式，成品统一保存为 MP4，默认存入系统 Downloads。
...
| 操作 | 按键 |
|---|---|
| 下载 → 压缩 → 字幕 → 任务 → 下载循环切换（未处理时） | Tab 下一页 / Shift+Tab 上一页 |
...
交互界面使用一个全屏应用，退出时恢复原终端画面。切换 Tab 会保留各页输入、光标、焦点和选项，包括字幕页的两个路径。Tab 栏水平居中，各项间隔三个空格，当前模式以强调色、加粗和方括号标记。上方使用三行字符 Logo；窄矮窗口自动改为单行 YTDock，并收起装饰留白。处理期间不能切换模式；同一用户只能运行一个实例。完成后可继续输入下一项。
...
视频有可用的原声语言字幕时，分辨率页显示 **S 字幕设置**，可选择「有字幕」；每个新视频默认「不带字幕」。没有可用的同语言字幕轨道时隐藏字幕入口。原声同语言的人工字幕优先、原生自动字幕后备，界面显示实际来源；不使用自动翻译轨、不做翻译或语音识别。
...
按 **Tab** 切换到 **[字幕]**，在两个输入框中分别填写视频和 **UTF-8 SRT** 路径。**↑↓** 切换输入框，**Enter** 检查，**Tab** 切换模式。支持空格、中文、配对引号、`~` 和 Finder 拖入路径；**Ctrl+O** 切换界面语言时保留输入。
...
处理和清理期间禁止切换 Tab 或语言。终端至少需要 **52 列 × 20 行**，出现尺寸提示时请继续扩大窗口；列表和详情支持滚动，缩放及切换语言后重新布局。`--help`、`--check`、安装和卸载继续使用普通终端输出。
