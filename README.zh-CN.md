...
`Release macOS` 工作流在推送到 `main` 时执行，也可在 `main` 手动触发。发布新版时，修改 `pyproject.toml` 的 `[project].version`，执行 `uv lock` 并一起推送。工作流不自动增加版本号；已发布版本直接跳过，不重新构建或覆盖安装包，新版本必须高于已有稳定版。

新版本先在 macOS 14 arm64 执行测试及隔离打包验收，再上传 ZIP、tar.gz 和各自 SHA256 文件。只有发布任务获得 `contents: write` 权限；标签指向实际通过测试的提交，先上传至草稿，四个文件全部上传成功后才公开为最新版。上传失败保留草稿，可在同一提交重试；不移动冲突标签，创建标签后若需修正源码请使用新版本号。

原有手动 `Prepare macOS release (no publication)` 工作流保留。CI 不验证实时 YouTube 下载、Apple 公证、另一台 Mac 或真实跨版本自升级。使用 GitHub 内置仓库令牌，无需配置个人访问令牌。
